...
\begin{document}
	...
	\newpage
	\part{Jedeme na to}
		V případě, že nejste příliš zdatní v českém jazyce, přečtěte si bonusovou kapitolu číslo \ref{typografie}, pojednávající o české typografii. Vlastně, \dots{} Podívejte se tam raději všichni.
		...
		\section{Základní kostra dokumentu}
			Každý dokument psaný v LaTeXu musí obsahovat hlavičku (preambuli) a samotný text. Vždy když vytváříte nový dokument, automaticky do něj vložte následující kód:
		...
		Jak už jste si mohli všimnout, příkazy LaTeXu se skládají ze znaků \texttt{\textbackslash{}, \{, \}}, proto není možné je do textu jen tak zapsat, aby se nevyhodnotily na příkaz. Speciálních znaků však existuje mnohem více\footnote{Spoustu symbolů lze najít na \url{http://latex.wikia.com/wiki/List_of_LaTeX_symbols}. Pro případ, kdy chcete zapsat symbol, pro nějž neznáte příkaz, jistě se vám bude hodit stránka \url{http://detexify.kirelabs.org/classify.html}.} než jen tyto tři. Následující tabulka ukáže, jak je psát.
		\\
		\\
		...
		\section{Písmo}
			\label{pismo}
			Následující tabulka osvětlí, jak lze měnit formát písma.
			\\\\
		...
		\newpage
		\section{Seznamy}
		Řekne-li se seznam (případně výčet), nejspíš ne úplně každý bude mít jasnou představu, o čem je řeč. Při čtení zdrojového kódu tohoto dokumentu jste si mohli seznamu všimnout například v kapitole č.\ref{pismo}, pojednávající o písmech.
		\\
	...
	\part{Typografie}
		...
		\paragraph{Interpunkce} -- tedy čárka, tečka, dvojtečka, středník, otazník, vykřičník, atd. se píše hned za znakem, mezera až za ní. Za nadpisem se tečka nepíše. Při dělení se dvojtečka píše s mezerami z obou stran, zatímco dvojtečka ve skóre se píše bez mezer.
